...
const Navbar = () => {
  ...
  return (
    <nav
      ref={navRef}
      className="w-full  bg-background/50 shadow-md z-50 backdrop-blur-md bg-opacity-95 dark:bg-opacity-95 "
    >
      <Container>
        <div className="flex justify-between h-16 items-center">
          <a href="/" className="flex items-center gap-3 cursor-pointer">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[8px] xs:text-[10px] sm:text-xs   xl:text-sm text-accent font-semibold uppercase tracking-widest mt-0.5">
                {lang === "en"
                ...
                return (
                  <div key={index} className="relative group py-2">
                    <button className="flex items-center gap-1 text-xs xl:text-sm font-medium hover:text-accent transition-colors py-2 cursor-pointer focus:outline-none">
                      <span>{item.name}</span>
                      <FiChevronDown className="w-3.5 h-3.5 group-hover:rotate-180 transition-transform duration-200 text-muted" />
                    </button>
                    <div className="absolute top-full left-0 w-48 bg-background border border-gray-200 dark:border-gray-800 rounded-lg shadow-xl py-2 opacity-0 scale-95 invisible group-hover:opacity-100 group-hover:scale-100 group-hover:visible transition-all duration-200 origin-top-left z-50">
          ...
          <div className="lg:hidden flex items-center">
            <button
              ...
              className="text-primary hover:text-accent focus:outline-none cursor-pointer"
            >
              <svg
                className="h-6 w-6"
                fill="none"
      ...
      {isOpen && (
        <div className="lg:hidden bg-background border-b border-gray-200 dark:border-gray-800 px-4 pt-2 pb-6 space-y-1">
          {menuItems[lang].map((item, index) => {
            if (item.submenu) {
              return (
                <div key={index} className="block">
                  ...
                  {isAcademicOpen && (
                    <div className="pl-6 pr-3 py-1 space-y-1 bg-gray-50 dark:bg-gray-950/40 rounded-md mt-1">
                      {item.submenu.map((subItem, subIndex) => (
                        <a
                          key={subIndex}
                          href={subItem.href}
          ...
          <div className="grid grid-cols-2 gap-2 pt-4 pb-2 border-t border-gray-100 dark:border-gray-900 items-center justify-items-center">
          ...
          <button className="w-full mt-2 bg-accent text-accent-foreground px-4 py-2.5 rounded font-semibold text-sm tracking-wider uppercase text-center shadow-md">
